...
int isNumberAdjacentToSymbol(struct matrix* matrix, unsigned int position, int length) {
    unsigned int x;
    unsigned int y;
    ...
    for (int i = -1; i <= length; i++) {
        for (int j = -1; j <= 1; j++) {
            ...
            if (isSymbol(charAtCoord(matrix, x + i, y + j))) {
                return 1;
            }
        }
    }
...
int findCode(struct matrix* matrix) {
    ...
    while (i < matrix->width * matrix->heigth) {
        if (!isdigit(charAtPosition(matrix, i))) {
            i++;
            continue;
        }
        char* end = 0;
        long number = strtol(matrix->container + i, &end, 10);
        int numberLength = end - matrix->container - i;
        if (isNumberAdjacentToSymbol(matrix, i, numberLength)) {
            code += number;
        } else {
        }
        i += numberLength;
